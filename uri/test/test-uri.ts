...
class Uri {
  @test async CreateFileUri() {
    assert.strictEqual(
      uri.CreateFileUri("C:\\windows\\path\\file.txt"),
      "file:///C:/windows/path/file.txt"
    );
    assert.strictEqual(
      uri.CreateFileUri("/linux/path/file.txt"),
  ...
  @test async CreateFolderUri() {
    assert.strictEqual(
      uri.CreateFolderUri("C:\\windows\\path\\"),
      "file:///C:/windows/path/"
    );
    assert.strictEqual(
      uri.CreateFolderUri("/linux/path/"),
      "file:///linux/path/"
    );
  ...
  @test async ExistsUri() {
    ...
    assert.strictEqual(
      await uri.ExistsUri(
        "https://raw.githubusercontent.com/Azure/azure-rest-api-specs/master/READMEx.md"
      ),
      false
    );
    assert.strictEqual(
      await uri.ExistsUri(uri.CreateFileUri(__filename)),
      true
    );
    assert.strictEqual(
      await uri.ExistsUri(uri.CreateFileUri(__filename + "_")),
      false
    );
  }
  ...
  @test async ResolveUri() {
    ...
    assert.strictEqual(
      uri.ResolveUri(
        "https://somewhere.com/file1.json?token=asd%3Dnot_really_a_token123%3D",
        "./file2.json"
      ),
  ...
  @test async ResolveURILinuxReleative() {
    assert.strictEqual(uri.ResolveUri("/home/foo", "."), "file:///home/foo");
  }

  @test async ResolveURIWindowsReleative() {
    assert.strictEqual(uri.ResolveUri("C:/home/foo", "."), "file:///C:/home/foo");
  }
